...
export function createCalendarSection(containerId) {
    ...
    container.innerHTML = `
      ...
      <div class="welcome-bar">
        <div class="welcome-content">
          <div class="welcome-greeting">📅 Calendar</div>
          <div class="welcome-status">
            <span class="status-badge">${monthItems.count} items this ${viewMode}</span>
            ${upcomingItems.overdue > 0 ? `
              <span class="status-badge" style="background: rgba(239, 68, 68, 0.15); color: var(--accent-danger);"
              >🔥 ${upcomingItems.overdue} overdue</span>
            ` : ''}
          </div>
        </div>
        <div class="welcome-actions">
          <div class="view-toggle" style="display: flex; gap: 0.5rem; margin-right: 1rem;">
            <button class="btn btn-sm ${viewMode === 'month' ? 'btn-primary' : 'btn-secondary'}" 
              onclick="setCalendarView('month')">Month</button>
            <button class="btn btn-sm ${viewMode === 'week' ? 'btn-primary' : 'btn-secondary'}" 
              onclick="setCalendarView('week')">Week</button>
            <button class="btn btn-sm ${viewMode === 'day' ? 'btn-primary' : 'btn-secondary'}" 
              onclick="setCalendarView('day')">Day</button>
          </div>
          <div class="calendar-nav">
            <button class="btn btn-sm btn-secondary" onclick="changePeriod(-1)">◀</button>
            <div class="calendar-month">${getPeriodLabel()}</div>
            <button class="btn btn-sm btn-secondary" onclick="changePeriod(1)">▶</button>
            <button class="btn btn-sm btn-text" onclick="goToToday()">Today</button>
          </div>
      ...
      <div class="card upcoming-card">
        <div class="card-header">
          ...
          <span class="upcoming-count">${upcomingItems.items.length} items</span>
        </div>
        
        ${upcomingItems.items.length === 0 ? `
          <div class="empty-state-small">
            <div class="empty-state-icon">📅</div>
            <div class="empty-state-text">No upcoming items</div>
          </div>
        ` : `
          <div class="upcoming-list">
            ${upcomingItems.items.slice(0, 10).map(item => renderUpcomingItem(item)).join('')}
            ${upcomingItems.items.length > 10 ? `
              <div class="upcoming-more">+${upcomingItems.items.length - 10} more items</div>
            ` : ''}
          </div>
        `}
      </div>
  ...
  function renderCalendarDays(firstDay, daysInMonth, daysInPrevMonth, monthItems, priorities, events) {
    ...
    return Array.from({ length: 42 }, (_, i) => {
      ...
      return `
        <div class="calendar-day ${!isCurrentMonth ? 'other-month' : ''} ${isToday ? 'today' : ''} ${isSelected ? 'selected' : ''} ${hasOverdue ? 'has-overdue' : ''} ${hasUrgent ? 'has-urgent' : ''}"
        ...
        >
          ...
          <div class="day-items">
            ${dayData.priorities.slice(0, 2).map(p => {
              const alert = getDueAlert(p)
              return `
            ...
            ${dayData.count > 4 ? `
              <div class="day-more">+${dayData.count - 4} more</div>
            ` : ''}
          </div>
        </div>
      `
  ...
  function renderDayDetails(dateInfo, priorities, events) {
    ...
    return `
      <div class="card day-details-card">
        <div class="day-details-header">
          <div class="day-details-title">${dateStr}</div>
          <div class="day-details-actions">
            <button class="btn btn-sm btn-primary" onclick="openEventModalForDate('${dateStr}')">➕ Add Event</button>
            <button class="btn btn-sm btn-text" onclick="closeDayDetails()">✕</button>
          </div>
        </div>
        ...
        ${dayPriorities.length === 0 && dayEvents.length === 0 ? `
          <div class="empty-state-small">
            <div class="empty-state-text">No items for this day</div>
            <button class="btn btn-sm btn-secondary" onclick="createEventOnDay(${year}, ${month}, ${day})" style="margin-top: 1rem;">
              ➕ Add Event
            </button>
          </div>
        ` : `
          <div class="day-details-list">
            ${dayPriorities.map(p => {
              const alert = getDueAlert(p)
              return `
                <div class="day-detail-item priority ${p.completed ? 'completed' : ''} ${alert?.type || ''}"
                     onclick="openEditPriorityModal(${p.id})">
                  <div class="detail-checkbox ${p.completed ? 'checked' : ''}"
                       onclick="event.stopPropagation(); togglePriority(${p.id})"></div>
                  <div class="detail-content">
                    <div class="detail-text">${escapeHtml(p.text)}</div>
            ...
            ${dayEvents.map(e => `
              <div class="day-detail-item event" onclick="openEventModal(${e.id})">
                <div class="detail-icon">🎉</div>
                <div class="detail-content">
                  <div class="detail-text">${escapeHtml(e.name || e.title)}</div>
                  <div class="detail-meta">
          ...
          <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--border);">
            <button class="btn btn-sm btn-secondary" onclick="createEventOnDay(${year}, ${month}, ${day})" style="width: 100%;">
              ➕ Add Event
            </button>
          </div>
        `}
  ...
  function renderUpcomingItem(item) {
    ...
    return `
      <div class="upcoming-item ${isPriority ? 'priority' : 'event'} ${alert?.type || ''} ${daysUntil < 0 ? 'past' : ''}"
        ...
        <div class="upcoming-content">
          <div class="upcoming-title">${escapeHtml(item.text || item.name || item.title)}</div>
  ...
  store.subscribe((state, path) => {
    if (!path || path.includes('priorities') || path.includes('events')) render()
  })
