    ...
    container.innerHTML = `
      ...
      <div class="m-card welcome-bar">
        <div class="welcome-content">
          <div class="m-title welcome-greeting">🖨️ Printers</div>
          <div class="welcome-status">
            ${errorCount > 0 ? `
              <span class="status-badge status-danger">🔴 ${errorCount} issue${errorCount > 1 ? 's' : ''}</span>
            ` : printingCount > 0 ? `
              <span class="status-badge status-primary">⚡ ${printingCount} printing</span>
            ` : `
              <span class="status-badge status-success">🟢 All online</span>
            `}
            <span class="m-caption status-badge">${onlineCount}/${printers.length} online</span>
          </div>
        </div>
        <div class="welcome-actions">
          ...
          ` : ''}
          <button class="m-touch btn btn-sm btn-secondary" onclick="refreshPrinters()" id="refreshPrintersBtn">
            🔄 Refresh
          </button>
        </div>
      </div>
    ...
    return `
      <div class="m-card printer-card ${printer.status} m-touch"
        ...
        <div class="m-card-header printer-header">
          ...
          <span class="m-caption printer-status-badge" style="background: ${statusConfig.color}20; color: ${statusConfig.color};"
