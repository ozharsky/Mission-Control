...
// Edge Decay Tracker - tracks how edge changes over time
class EdgeDecayTracker {
  constructor() {
    this.edgeHistory = new Map(); // ticker -> array of {timestamp, edge}
    this.maxHistory = 10; // Keep last 10 measurements
  }
  ...
  record(ticker, edge) {
    ...
    history.push({ timestamp: Date.now(), edge });
  ...
  getEdgeChange(ticker) {
    const history = this.edgeHistory.get(ticker);
    if (!history || history.length < 2) return 0;
    const first = history[0];
    const last = history[history.length - 1];
    return last.edge - first.edge;
  }

  getAvgEdge(ticker) {
    const history = this.edgeHistory.get(ticker);
    if (!history || history.length === 0) return 0;
    const sum = history.reduce((acc, h) => acc + h.edge, 0);
    return sum / history.length;
  }

  getTrend(ticker) {
    const change = this.getEdgeChange(ticker);
    if (change > 5) return 'improving';
    if (change < -5) return 'decaying';
    return 'stable';
  }

  printDecayReport() {
    console.log('\n📊 EDGE DECAY REPORT:');
    const sorted = Array.from(this.edgeHistory.entries())
      .filter(([_, h]) => h.length >= 2)
      .sort((a, b) => this.getEdgeChange(b[0]) - this.getEdgeChange(a[0]));
    ...
    console.log('\n  📈 TOP IMPROVING:');
    sorted.slice(0, 3).forEach(([ticker, history]) => {
      const change = this.getEdgeChange(ticker);
      const avg = this.getAvgEdge(ticker);
      if (change > 0) {
        console.log(`    ${ticker}: +${(change || 0).toFixed(1)}% (avg: ${(avg || 0).toFixed(1)}%)`);
      }
    });

    console.log('\n  📉 TOP DECAYING:');
    sorted.reverse().slice(0, 3).forEach(([ticker, history]) => {
      const change = this.getEdgeChange(ticker);
      const avg = this.getAvgEdge(ticker);
      if (change < 0) {
        console.log(`    ${ticker}: ${(change || 0).toFixed(1)}% (avg: ${(avg || 0).toFixed(1)}%)`);
...
async function main() {
    ...
    try {
      ...
      for (const m of markets.slice(0, CONFIG.maxMarketsPerSeries)) {
        ...
        if (yesPrice >= CONFIG.minPrice && yesPrice <= CONFIG.maxPrice && 
          ...
          edgeDecayTracker.record(m.ticker, edgeCalc.edge);
  ...
  edgeDecayTracker.printDecayReport();
